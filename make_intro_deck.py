...
역할 분담 (2026-09-14 운영자 결정):
  · 메일(docs/사내공유_메일_20260913.md) = **무엇을 하는 시스템인가** — 기능을 네 범주로 요약
  · 이 자료                              = **어떻게 쓰는가** — 화면별 사용법, 실제 캡처 중심
목차를 메일과 똑같이 맞추지 않는다(같은 목차를 두 번 읽히는 것은 낭비다). 다만 **사실이 어긋나면
안 된다** — 수치(244명·18개·매시간), 인용 표시 이름, 보안 문구, 팀 확대 관련 서술은 메일을 따른다.
특히 아직 결정되지 않은 것(전사 확대·중앙 단일 운영·팀별로 나눌 항목)은 이 자료에도 적지 않는다 —
첨부가 메일보다 앞서 약속하면 그 차이가 곧 분쟁이 된다.
고치는 순서는 메일 원본 → 이 스크립트. AI 호출 없음.
...
def build():
    ...
    # ── 04-2 조문 단위로 보기 ─────────────────────────────────
    s = blank(prs); head(s, '① 법령 관계도 — 조문 단위로 보기', '법령끼리가 아니라 조문끼리 어떻게 물려 있는지', 5)
    text(s, MARGIN, Inches(1.68), BODY_W, Inches(0.6),
         [('지도에서 법령을 누르면 그 법의 조문 지도가 열립니다. 실제 검토는 「전파법 ↔ 시행령」이 아니라 '
           '「제19조가 시행령 제18조에 위임한다」 수준에서 이뤄지기 때문입니다.',
           12.5, False, INK)], line_spacing=1.35)
    cards(s, [
        ('조문 본문을 읽습니다',
         '조문 안의 「…법 제N조」 인용을 뽑아 조문과 조문 사이에 선을 긋습니다.\n\n'
         '현재 조문 인용 3,186건.'),
        ('줄임말도 풀어서',
         '「같은 법」·「같은 법 시행령」·「시행규칙」처럼 앞 문장을 받아 쓰는 표기와 '
         '「제6·18조」·「제67~68조」 같은 묶음 표기도 각각의 조문으로 풀어 잇습니다.'),
        ('검색어로 닿지 않는 곳',
         '벌칙·사실조사 조항은 질문에 쓰는 낱말이 안 들어 있어 검색으로는 나오지 않습니다.\n\n'
         '어느 조문이 그 조문을 걸고 있는지는 인용선을 따라가야 보입니다.'),
    ], Inches(2.5), cols=3, card_h=Inches(2.6), head_size=15)
    note(s, '조문 번호는 개정으로 바뀝니다 — 지도에 그려진 조문은 현재 지식베이스에 담긴 판(版) 기준입니다.')
    ...
    rows(s, [
        ('[원문 확인됨: 전파법 제21조]', '기계가 원문과 대조해 맞음을 확인했습니다'),
        ('[원문 확인 안 됨]', '조문을 못 찾았거나, 대조할 수 없었거나, 원문과 달라 보입니다 — 원문 링크로 직접 확인하십시오'),
    ...
    note(s, '[원문 확인됨]은 인용이 맞다는 뜻이지 결론이 맞다는 보증은 아닙니다 — 대외로 나가는 문서는 원문을 한 번 더 확인하십시오.\n'
            '답이 틀리거나 근거가 빠졌으면 답변 아래 👍👎로 남겨 주십시오 — 기록에 남아 판정 기준문 손질에 쓰입니다.')
    ...
    rows(s, [
        ...
        ('국회', '법안 발의·처리 경과와 입법예고 · 과방위 회의록(2016년 20대 국회부터) · 인물 244명의 발언 이력과 쟁점별 입장 요약'),
    ...
    # 문구는 메일 4항과 글자 단위로 맞춘다. 아직 결정되지 않은 것(전사 확대·중앙 단일 운영·
    # 팀별로 나눌 항목)은 적지 않는다 — 첨부가 메일보다 앞서 약속하면 그 차이가 곧 분쟁이 된다.
    text(s, MARGIN, Inches(1.68), BODY_W, Inches(0.6),
         [('현재는 전파·전기통신 및 과방위를 중심으로 운영하고 있습니다. 법안 수집 키워드에는 '
           '개인정보·인공지능·플랫폼·데이터·클라우드·메타버스가 이미 들어 있습니다.',
           12.5, False, INK)], line_spacing=1.35)
    cards(s, [
        ('어떻게 나눌지 정리 중입니다',
         '팀마다 소관 법령과 상임위, 중요하게 보는 뉴스가 다릅니다.\n\n'
         '어디까지 공통으로 두고 어디부터 팀별로 나눌지는 지금 정리하고 있으며, 정해지는 대로 순차적으로 열어 드리겠습니다.'),
        ('파일럿으로 먼저',
         '관심 있는 팀은 파일럿으로 먼저 쓰실 수 있습니다.\n\n'
         '소관 법령·고시 목록만 주시면 시작합니다.'),
        ('보안',
         '보안상 사내 문서·대외비 자료는 입력하거나 업로드하지 말아 주십시오.\n\n'
         '사내 시스템 구축은 별도로 준비 중입니다.'),
    ...
    rows(s, [
        ('과방위 회의록', '2016년 6월부터 회의 546회 · 전파·통신 관련 발언 요지 6,743건 (상임위 · 국정감사 전문)'),
        ('인물', '244명 — 의원 127명 · 정부 인사와 기업 증인 117명. 공개된 회의록 발언에 한합니다'),
        ('국회 법안', '669건 — 발의부터 처리까지 단계 추적'),
        ('정책 뉴스', '현재 1만 1천여 건 보관 · 매시간 수집 · 60일 경과분은 정리'),
        ('법적 용어', '조문에 정의된 법률 용어 1,370개 — 원문 그대로'),
        ('법령 관계도', '법령·고시 1,230여 건 + 주제 73개, 그 사이의 위임·인용 관계 (전파·전기통신 분야)'),
