...
공백 제거 기준으로 API본과 대조해 **A. API가 상위집합 68건 / B. 대형 법률 4건**(PDF가 더 큰 원인이 "현행+시행예정 중복 수록"임을 규명) / **C. 소형 22건**(조건표가 이미지로만 와서 실제로 PDF에만 있는 내용이 있음)으로 나눴고, 운영자 승인을 받아 **A+B 72건 5,939청크를 삭제**했다. superseded가 8,588 → 2,649청크로 줄었다. C는 용량이 0.5MB로 미미해 남겼다.
