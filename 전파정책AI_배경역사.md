...
**설계 — 중복 방지가 핵심.** 큐 구조를 바꾸지 않고 얹었다. `telegram_subscribers.urgent_now`
(기본 false) 신설 → 크롤러가 긴급을 큐에 적재한 **직후** `subscriber_notify.send_urgent_now()`가
urgent_now=true인 구독자에게만 구독봇 토큰으로 즉시 발송하고, **그 구독자의 last_urgent_sent_at을
현재로 갱신**한다. 정시 발송(send-subscriber-briefing)은 원래 이 값 이후 큐만 집어가므로 같은 건이
다시 나가지 않는다 — 발송 로직을 두 벌로 만들지 않고 기존 필터를 그대로 활용한 것. 발송 실패 시엔
갱신하지 않아 정시에 정상 수신된다(fail-open). '평일만' 선택자는 주말 즉시 발송에서 제외.
설정 화면에는 콘텐츠 3종 아래에 `⏰ 긴급은 즉시 받기 (야간 포함)` 체크박스를 추가(callback `u:now`).

**배포.** telegram-webhook 재배포(npx supabase@latest, --no-verify-jwt). Docker 미실행 경고는
