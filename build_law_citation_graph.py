...
# 인용 엣지를 만들지 않을 법령 — 두 부류다.
#  ① 부칙·개정문 상용구가 기계적으로 인용하는 절차 규정 — 관계도 가치 없음
#  ② **폐지된 법령** — 현행법 부칙의 폐지·경과·타법개정 문구("…을 폐지한다",
#     "종전의 …에 따른다", "…를 「전기통신사업법」으로 한다")에만 남아 있어,
#     본칙에 살아 있는 관계가 하나도 없는데도 노드가 되살아난다.
#     단통법은 2026-04-28 폐지되고 조항이 전기통신사업법으로 이관됐는데
#     전기통신사업법 부칙 인용만으로 27회짜리 엣지가 생겨 있었다(2026-09-14 실측).
#     ※ 부칙 청크를 통째로 제외하는 방법도 있으나 부칙 인용이 전체 인용표시의 11.6%
#       (1,958/16,918)라 영향 범위가 커서, 폐지 법령만 이름으로 막는다.
# (비교는 nrm_key로 한다 — "훈령․예규 …"처럼 점 이형으로 들어오면 원문 비교는 빠져나간다)
CITE_BLOCKLIST = {
    '훈령·예규 등의 발령 및 관리에 관한 규정',
    '이동통신단말장치 유통구조 개선에 관한 법률',            # 폐지 2026-04-28 → 전기통신사업법 이관
