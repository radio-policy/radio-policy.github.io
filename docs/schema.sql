...
-- 1-14b) 법적 용어 정의 (#156, 2026-09-11) --------------------------------------
-- 현행 법령·고시의 정의 조문(제N조(정의)·(용어의 정의)·(용어정의)·(용어의 뜻))에서 호 항목을 원문 그대로 추출.
-- 파생 테이블 — law_terms_sync.py 가 매일 11:00 KST(law_crawl.yml) 전량 재생성(upsert → synced_at 미갱신 행 삭제). SQL 직접 편집 금지.
create table if not exists public.law_terms (
  id             bigint generated by default as identity primary key,
  term           text not null,                 -- 따옴표 안 용어 (괄호 병기·'(이하 …)' 제거)
  term_key       text not null,                 -- 정규화: 공백·괄호부 제거, lower (대시보드 그룹·검색 키)
  term_alias     text,                          -- 괄호 병기: 無線局 / Spurious 發射 / accounting rate
  law_name       text not null,                 -- law_watch.parse_doc_name().law_name
  full_name      text,
  law_type       text not null default '기타',  -- 법률|대통령령|부령|규칙|고시|훈령|예규|공고|기타
  doc_name       text not null,                 -- document_chunks.doc_name (판 스탬프 포함)
  doc_category   text,
  article_no     text not null,                 -- '2조(정의)'
  article_key    text,                          -- '2조' / '19조의2'
  item_no        text not null,                 -- '1' / '4의2' / '1항' / '본문'
  definition     text not null,                 -- 항목 전문(용어 포함, 가.나. 목 포함, <개정…> 꼬리 제거)
  law_no         text,                          -- '제21065호'
  effective_date text,                          -- 'YYYYMMDD'
...
-- insert/update/delete 정책 없음 → service_role(law_terms_sync.py) 전용
...
create index if not exists law_terms_term_key_idx on public.law_terms (term_key);   -- #156
