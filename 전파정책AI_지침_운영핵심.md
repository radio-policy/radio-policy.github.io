...
- **`.range()` 페이지 조회의 정렬을 겹칠 수 있는 열(`published_at`·`created_at`·`meeting_date` 등) 하나로 끝내지 말 것 — 끝에 유일 열(`id`, 키가 url인 `news_screen_cache`는 `url`)을 붙일 것 / `crawler._fetch_all_rows`의 `order` 인자를 빼지 말 것 (#233, 2026-09-26, 운영자 결정 A+B)** — 정렬이 없거나 겹치면 요청마다 행 순서가 달라져 페이지 경계에서 행이 빠지거나 두 번 온다(오류 없음). 사내 `export_news.py`가 `created_at` 단독 정렬로 기사 28건을 흘린 것이 확정 사례다(#232 보론). 외부판에선 아래 #66 규칙이 모범으로 든 `_fetch_all_rows`가 정작 정렬이 없었다 — news_feed 14,068행·news_screen_cache 30,919행을 15·31번 나눠 받는데, 빠지면 이미 알린 기사 재알림(8/3 사고 유형)·운영자가 지운 기사 재수집·선별 AI 재판정. 대시보드 뉴스 목록은 발표 시각이 겹치는 기사가 4,141행이다(1·2단계 요청의 정렬은 반드시 같게). 문서 안 `chunk_index` 정렬(app.js·law_diff_gen·okf_refresh 등 10곳)은 실DB 겹침 0이고 `kb_store.insert_chunks`가 같은 번호 중복을 잡아(#218) 고치지 않았다 — 다시 제안하지 말 것. (배경역사 #233)
