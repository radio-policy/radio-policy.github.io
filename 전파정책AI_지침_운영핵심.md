...
```
C:\Users\SKTelecom\Desktop\frequence\radio-policy-ai\      (회사 노트북 — 예약작업 비활성 예비. PC 예약작업 본선은 집 PC C:\Claude\Radio-policy\radio-policy-ai, 24시간·사용자 계정, 2026-09-20 #178·#179)
...
| 독립 | PC Windows 작업 스케줄러 — **집 PC(24시간) 본선, 회사 노트북은 비활성 예비**(#179) | 정시 | 집 PC |
...
- **PC 예약작업(gov 체인 16:30·본문 재수집 매시 22분·브리핑 예비 09:40·법안 요약 10:30·CRMS 매월 1일 16:30)은 2026-09-20부터 집 PC(24시간, 사용자 계정)가 본선이다(#179).** 회사 노트북에는 같은 작업 5개+로컬미리보기가 **등록만 남긴 비활성 예비**로 있다 — 집 PC가 멀쩡한 동안 다시 켜지 말 것(두 체인이 겹치면 assembly_minutes 임포트 충돌·Haiku 비용 2배). 집 PC 장애 시에만 `schtasks /change /tn <이름> /enable`. 외부 워치독이 `last_gov_notice_run`(26h)·`last_refetch_run`(3h)을 보고 "집 PC 확인"으로 알린다.
...
- **외부** `health_watchdog.py`(GitHub Actions, Supabase 독립) — 뉴스·브리핑 신선도 + daily_crawl·morning_briefing·law_crawl·assembly_crawl 4종 워크플로우 성공 이력 + "Supabase 접속 불가" 자체 감지 + **PC 예약작업 heartbeat 2종**(`last_gov_notice_run` 26h·`last_refetch_run` 3h, 넘으면 "… 마지막 실행 N시간 전 — 집 PC 확인", #179). PC 작업은 Actions run 이력이 없어 heartbeat로만 볼 수 있고, 내부 watchdog_scan도 같은 키를 보지만 Supabase cron이 서면 함께 서므로 외부에도 둔다.
...
- **PC heartbeat(`last_gov_notice_run`·`last_refetch_run`) 경고는 집 PC부터 본다(#179)** — 회사 노트북 작업은 비활성이라 이어받지 않는다. 집 PC 전원·네트워크 → 작업 스케줄러 마지막 실행 결과 → `gov_crawler_log.txt`·`refetch_log.txt` 순.
...
- **회사 노트북의 비활성 예약작업 5개(전파정책_정부크롤러·RadioPolicy-RefetchContent·BriefingBackup·AssemblySummary·전파정책_CRMS동기화)와 로컬미리보기를 집 PC가 살아 있는 동안 다시 켜지 말 것(#179, 2026-09-20)** — 집 PC(24시간)가 본선이다. 둘이 같이 돌면 gov 체인의 assembly_minutes가 같은 회의를 두 번 등재할 수 있고(임포트 단일·순차 규칙), 판정·요약 Haiku 호출이 2배가 된다. 로컬미리보기(http.server 0.0.0.0:8000)는 `.env`·로그를 네트워크에 노출하므로 예비로도 켜지 않는다. 집 PC 장애 시에만 `schtasks /change /tn <이름> /enable`로 되살리고, 복구 후 다시 끈다.
