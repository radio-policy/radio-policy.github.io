...
- **식별자는 (MST, 시행일)** — 같은 MST가 시행일별로 다른 통합본을 갖는다(정보통신망법 MST 285199 → 20261001 179조 / 20270401 180조). 조문 취득은 `target=eflaw` + `MST` + **`efYd` 필수**(efYd 없이 부르면 빈 응답). **현행본 적재(`sync_one`·`reingest_one`)도 예외 아님** — `target=law`+MST만 쓰면 그 MST의 가장 늦은 시행일 통합본이 와서 현행 자리에 미래 조문이 들어간다(#207).
...
- **법령 조문 취득에서 `fetch_law_articles(mst)`처럼 시행일을 빼지 말 것 — 현행본 적재도 `ef_date=enf` 필수 (#207, 2026-09-24)** — MST만 주면(target=law) 법제처가 같은 MST의 **가장 늦은 시행일 통합본**을 준다. `sync_one`·`reingest_one`이 그렇게 받아, KB 현행 「전기통신사업법(제21652호)(20260519)」에 10/1·11/20 시행 조문(제32조의20~22 신설, 대주주 변동 보고, 가입제한서비스 고유식별정보)이 들어가 있었다 — 자문이 미시행 조문을 현행으로 인용할 수 있었고, law_diffs 6번은 이 오염본을 기준으로 삼아 "10/1에 조항 삭제"라는 반대 요약을 냈다. 현행 268건 대조에서 항공안전법·공정거래법·전자상거래법(·시행령)·정부조직법 등도 의심(정밀 확인 전)
