...
- **법안 알림은 실행당 한 통으로 묶고, 구독자는 '인지·행동·준비' 세 순간만(2026-09-09, #140 운영자 결정)**: ① 신규 발의(`format_new_bills_batch`, 운영자+구독자 같은 한 통) ② 의견등록 가능(기존 입법예고 패스 알림: 시작·D-3) ③ 통과 가능성 높은 단계 진입 + 폐기(`format_status_batch(changes, SUBSCRIBER_STATUS)` — 위원회 의결·법사위·본회의·가결·정부이송·공포·대안반영폐기·부결·철회). **상정(소관위 회부→심사중)은 구독자에게 보내지 않는다**(할 일이 없는 단계; 대시보드 스트립·'최근 7일 단계 이동'이 맡음). 운영자 봇은 `NOTABLE_STATUS` 전부(상정 포함)를 같은 한 통 형식으로(`send_operator_only`). 묶음은 전이 종류별 그룹(의미 큰 순: 가결→의결→법사위→기타→폐기), 그룹당 10건 + '외 N건', 3,300자 넘으면 남은 그룹은 건수만. 법안 1건당 1통을 다시 만들지 말 것 — 9/9 상정 57건이 구독자 7명에게 수백 줄로 간 사고.
