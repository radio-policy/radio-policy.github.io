...
- **긴급도 판정(`classify_urgency`)에서 네이버 요약(`_URGENCY_SUMMARY`) 전달을 빼지 말 것 (#188, 2026-09-24)** — 10분 크롤은 Actions(미국 IP)라 본문 칸이 비어 있어, 요약이 없으면 긴급도가 **제목 한 줄**만 보고 매겨진다. 요약은 DB에 넣지 않는다(content에 넣으면 refetch_content의 '100자 미만=재수집' 조건이 깨진다). **2026-10-08 판정**: ① 긴급률이 기준 8.5~11.9%(09-07~09-21 주간) 대비 ±5%p 안 ② 세션 블라인드 채점 — 변경 전(제목만)·후(요약 포함) 기사 각 30건을 세션 에이전트가 요약까지 보고 어느 쪽인지 모른 채 채점, 크롤러 판정과의 일치율이 후가 같거나 높으면 유지, 아니면 되돌림. 운영자 수동 수정(`importance_feedback`, 월 20건 안팎)은 표본이 적어 판정 근거로 쓰지 않는다.
