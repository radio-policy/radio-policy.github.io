...
- **이미 등재된 예정본은 재적재하지 않는다.** 운영자가 PDF로 올려둔 시행예정본은 doc_name 끝에 `.pdf`가 붙고 **별표까지 포함돼 API본보다 내용이 많다** — 중복 생성 대신 기존 문서에 연결한다.
...
python law_sync.py --reingest-laws             # 실행(별표 표 있는 문서는 자동 제외)
python law_sync.py --reingest --doc-name "<문서명>"   # 1건만
```

- **법률(법률 계열)은 재적재해도 손실이 없다.** 별표는 시행령·시행규칙·고시에 있고 법률 본문에는 거의 없다 — 대상 10건 전부 별표 표 본문 0건으로 실측 확인.
- **시행령·시행규칙·고시는 재적재하지 말 것.** 법제처 조문 API는 **별표를 주지 않는다**(적합성평가 고시 PDF 166청크 → API 48청크). 전파법 시행령·시행규칙에는 행정처분기준 별표가 실제로 들어 있다(다만 PDF 표 추출이 열을 뒤섞어 사실상 판독 불가 상태다 — 재적재 여부는 이 점을 감안해 개별 판단).
...
- **구본은 삭제하지 않는다.** 문서명이 같으면 뒤에 ` [PDF원본]`을 붙여 `superseded`로 내린다 — 되돌릴 수 있고, 별표가 필요하면 그 문서를 다시 보면 된다.
