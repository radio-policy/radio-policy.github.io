...
- **첫 화면은 주소로 지정한다 (2026-09-15, 배경역사 #170)**: `?p=<화면>` 또는 `#<화면>`으로 열면 그 화면에서 시작한다(`?p=lawmap`·`#issuemap` 등, 허용 값은 `PAGE_TO_NAV` 키뿐이며 없는 이름은 무시하고 뉴스로 시작). **대외 공유·실적 제출 링크는 관계도(`?p=lawmap`)로 준다** — 뉴스 화면은 어느 AI 도구에나 있어 첫인상에서 이 시스템의 것이 드러나지 않는다. 부수 효과로 뉴스가 아닌 화면으로 시작하면 뉴스 1만 건 조회를 건너뛰어 첫 화면이 빨리 뜬다(뉴스는 `go('news')`가 그때 불러온다). 파라미터 없는 평소 접속은 종전대로 뉴스.
- **관계도 머리말 숫자는 화면에 그린 데이터로 센다 (2026-09-15, #170)**: `updateLawmapCaption()`이 `_lawMapNodes`·`_lawMapEdges`를 세어 "법령·고시 N종 · 연결 N건 · 주제 N개"를 쓴다. 별도 집계 쿼리를 두지 않는 이유는 화면의 노드·선과 숫자가 어긋나면 그 자리에서 틀린 것이 되기 때문이다. 머리말이 차지한 높이만큼 `.lawmap-graph` 높이에서 뺐다(`100vh - 400px` → `- 442px`) — 머리말을 고치면 이 값도 같이 본다.
