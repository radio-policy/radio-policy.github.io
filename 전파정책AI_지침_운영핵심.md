...
[매일 11시] law_sync.py --promote   시행일 도래한 pending → current 승격(+직전 current → superseded)
            ...
            okf_drift_check.py --notify  OKF 요약 ↔ 조문 판 대조, 어긋남 집합이 바뀐 날만 알림(#197)
...
조문(`document_chunks`)이 현행화돼도 **OKF 요약(`kb_documents`)은 자동으로 따라오지 않는다.** 요약이 옛 판을 설명하면 자문이 폐지된 조문을 근거로 답한다 — 조문 현행화와 반드시 짝으로 처리할 것. 어긋난 목록은 `okf_drift_check.py --dry-run`이 보여 주고(11:00 체인이 매일 대조해 집합이 바뀐 날 알림, #197), 교체 완료 텔레그램에도 같이 실린다.
