...
- **설정 화면에 별도 관리자 비밀번호를 되살리지 말 것 — 관문은 계정 역할 하나다** (#104 5단계, 2026-08-20). 로그인 계정에 admin 역할이 생긴 뒤 비밀번호를 함께 두면 같은 사람에게 두 번 인증을 요구하는 이중 잠금이 된다. 화면 해시 비교(`ADMIN_PWD_HASH`)는 제거했고, 관리 RPC 10종은 서버의 `admin_ok(p_pwd)`가 **`is_admin()` 또는 비밀번호 해시** 중 하나만 맞으면 통과시킨다(비밀번호 경로는 구버전 캐시 번들 호환용으로 당분간 존치 — 해시 상수는 이제 `admin_ok` 한 곳에만 있다). 프런트는 `p_pwd: ''`를 넘긴다. (배경역사 #104)
