...
| kb_documents | 법령·규제 **요약/실무 문서**(regulatory-kb OKF 번들, 문서당 1행). concept_type·law_type·law_number·enforcement_date·status(current/superseded)·body_md 컬럼. path 유니크(정체 키). **document_chunks(조문 원문)와 별개 레이어** — 조문 인용은 그쪽, 요약·적용범위·실무는 이쪽. RLS+anon select. **법안 요약(concept_type='Bill', 2026-09-04·#121)**: 국회 계류 중인 법률안 OKF — `path`는 `bills/<year>/<의안번호>.md`, `family='bills'`, `law_type='의안'`, `law_number`=의안번호, `enforcement_date`=발의일, `dedup_key`='제목\|의안#의안번호'(같은 이름 개정안이 제안자만 다르게 여러 건 공존하므로 법령과 달리 의안번호를 붙인다), `status`는 항상 'current'(RPC only_current 필터 통과용). 확정 법령이 아니므로 자문은 `[법안요약 · 국회 계류 — 확정 법령 아님]` 라벨로 별도 표시하고 조문 다리에서 제외한다(rag.ts·app.js `isBillRow`) |
...
- **법안 요약(Bill) — 신규 개념**: `regulatory-kb/bills/<year>/<의안번호>.md`, `concept_type='Bill'`(상세 규칙은 위 DB 표 kb_documents 행 참조). 본문은 요약/제안이유/주요내용/통신사업자 영향/처리 현황 섹션으로 구성하고, `bills/` 경로는 법령용 `# Citations` 게이트 대신 그 섹션 존재 여부를 검사한다(`import_regulatory_kb.check_body_complete`의 `bills/` 분기 — `# 제안이유`·`# 주요내용`·`# 통신사업자 영향` 3섹션 필수 + 말미 완결성). 109건 등재, kb_chunks 545청크. 자문 경로 2벌(`rag.ts`·`app.js`) `buildKbContext()`가 `concept_type==='Bill'`이면 라벨을 `[법안요약 · 국회 계류 — 확정 법령 아님]`으로 바꾸고 안내문을 붙이며, `answerLawQuery`의 조문 다리(법령명 → 현행 조문 조회)에서는 Bill을 제외한다(전파법 일부개정법률안 제목으로 현행 조문을 끌어오면 안 되므로). `system_prompt.js` [국회 동향 활용 원칙]에 ⑥ 추가 후 `sync_system_prompt.py` 실행(8,815자) + telegram-webhook 재배포. **법안은 법령 관계도(law_graph) 노드가 아니다** — 설계 유지, `law_diff_gen.py`도 국회분은 여전히 "KB 등재 법령만" 대상(#86)이라 무변경.
...
- **법안 요약(kb_documents `concept_type='Bill'`)을 확정 법령처럼 인용하지 말 것** — 국회에 계류 중인 법률안 요약이지 시행 중인 법령이 아니다. 라벨 `[법안요약 · 국회 계류 — 확정 법령 아님]`·시스템 프롬프트 [국회 동향 활용 원칙] ⑥이 방어선이며, `rag.ts`·`app.js` `buildKbContext()` **두 벌을 동시에** 고칠 것(한쪽만 고치면 봇과 대시보드가 같은 질문에 다르게 답한다). (배경역사 #121)
