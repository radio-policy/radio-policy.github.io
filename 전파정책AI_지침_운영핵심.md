...
- **DB 설계도의 정본은 `docs/db_baseline/`(#227, 2026-09-26)** — `py -3.12 tools_db_baseline.py`(읽기 전용, Management API)가 실DB 카탈로그에서 확장·시퀀스·표·외래키·함수·뷰·인덱스·트리거·정책(public+storage)·권한(객체마다 전부 회수 → 지금 권한 부여)·역할 설정·cron·버킷·Vault **이름**과 참고용 마이그레이션 이력을 뽑는다. 데이터는 없고 비밀(봇 토큰·Vault 값·운영자/구독자 chat_id·메일·해시)은 가린다 — 가린 뒤에도 비밀 후보나 .env 값이 남으면 파일을 하나도 안 쓰고 멈춘다. **DB를 바꾼 세션은 커밋 전 `tools_release.py` ⑦이 '설계도가 실DB와 다름'을 알리면** `tools_db_baseline.py` → 바뀐 파일만 명시 add. 복구 순서는 그 폴더 `README.md`(번호 순서 → Vault 값 재입력 → cron → Edge·Secrets·Auth). `docs/schema.sql`은 폐지(주석만 — 옛 anon 쓰기 모델이라 새 설치 시 #226 봉쇄가 풀렸다). 복구 연습(브랜치 적용)은 아직 안 했다.
