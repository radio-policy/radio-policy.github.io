...
- **"조용한 실패"를 만들지 말 것 — 2026-08-03에만 4건이 같은 유형이었다.** 에러 없이 결과만 틀리는 코드는 며칠씩 발견되지 않는다. 실측 사례: ①뉴스 1,000건 초과 시 조회가 잘려 이미 알림한 기사를 재발송 ②`.range()` 페이징에 `.order()`가 없어 관계도 노드가 매 실행 오삭제 ③refetch가 해외 기사를 수집 즉시 삭제 ④뉴스 클러스터가 연쇄 병합으로 91건 한 덩어리. **판정·필터·삭제 로직을 만들면 "틀렸을 때 무엇이 보이는가"를 먼저 정하고, 안 보이면 로그를 남길 것.**
