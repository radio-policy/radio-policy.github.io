...
- **조문 단위 보기 (2026-09-06 신설 — 배경역사 #125, `lawmap_articles.js`)**. 주제 포커스의 **자세한 그림** — 기본은 종전 법령 단위(큰 그림)이고 상태줄의 "조문 단위로 자세히 보기" 버튼으로 내려간다(운영자 결정: 먼저 어떤 법이 걸려 있는지, 그다음 조문 단위로. localStorage `lawmap_article_mode`가 '1'일 때만 기본 켬). 동그라미=조문, 법령=조문을 담는 배경 상자(캔버스 `beforeDrawing`), 주제→근거 조문은 파란 점선, 회색 실선은 **근거 조문 본문이 다른 근거 조문을 실제로 인용하는 곳에만** 조문 라벨(`37조의2③ → 사업법 38조`)과 함께. 전체 인용망·법령 포커스는 종전(법령 단위) 그대로. 상태줄 토글 "조문 단위 보기 켬/끔"(localStorage `lawmap_article_mode`)으로 종전 주제 화면과 전환. `renderLawMapGraph` 첫 줄의 `lmaShouldHandle` 훅 한 곳에서 분기하고, 모델 구축이 실패하면 `_lmaBypass`로 종전 화면에 폴백(fail-open — 화면이 비는 것보다 낫다). 데이터는 노드 카드가 쓰는 `document_chunks` 조문 청크만, AI 호출 0회, 조회는 4개씩 병렬.
  ...
  - **위임 점선 폴백**: 하위법령·고시 근거 조문이 상위법(또는 상위 시행령) 조문을 본문에서 인용하지 않는데 두 노드 사이에 delegation/thdcmp/family 엣지가 있으면 첫 근거 조문 사이를 "위임" 점선으로 잇는다("법에서 위임된 사항"처럼 조문 번호 없는 위임 문구 대비). 상위법은 이름 접두(…시행령/시행규칙 → 법률) → 위임·계열 엣지 순으로 찾는다.
