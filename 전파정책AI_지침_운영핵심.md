...
- **app.js 함수를 `supabase/functions/_shared/*.js`로 새로 옮기면서(공용화) 사내판에 먼저 알리지 않고 넘어가지 말 것 (#215-보론, 2026-09-26 사내 요청)** — 사내 콘솔 `ported.js`는 사내 `scripts/port_console_js.py`가 외부판 app.js에서 필요한 선언을 뽑아 만든다. 함수가 `_shared` 파일로 가고 app.js에 `var X = RagCore.X;` 같은 별칭만 남으면, 다시 만든 ported.js는 그 파일이 사내에 없을 때 로드 중 멈춘다(사내 스크립트는 09-26 전까지 `RagCore.` 꼴 전역을 못 잡아 경고도 없었다 — 사내가 고쳐 이제 별도 파일이 필요한 전역을 따로 알린다). 09-26 사내는 rag_core.js를 콘솔 `static/`에 원본 그대로 복사해 ported.js보다 먼저 싣는다(사내 bd392f9, 외부 원본과 해시 동일). 사내 콘솔이 쓰는 것은 `extractKeywords` 하나(관계도 질문 매칭) — 이 함수를 고칠 때도 인계에 한 줄. (배경역사 #215 보론)
