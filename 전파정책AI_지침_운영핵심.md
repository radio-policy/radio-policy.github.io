...
- **비용 구조(2026-08-13 실측)**: 평상시 **월 ~8만 원**(Haiku 판정이 대부분, 자문 Sonnet은 월 ~1.5만 원).
  ...
  2,048은 오류, #152 정정). 긴급도 콜은 9/13 기준문 강화로 접두가 1.5~1.9k → **4,750토큰**이 되어 캐시가 걸리게 됐다: system을
  [기준문+증류 규칙+등급별 균형 사례 4건](캐시 1h) + [제목 유사 사례 ≤5건](기사마다 다름, 캐시 밖) 두 블록으로 나눴다
  (`_feedback_fixed_block`/`_feedback_similar_block`). 콜당 정가 입력 ≈4,700 → 캐시 읽기 ≈4,750×0.1 + 유사 사례·제목 ≈300 →
  월 ≈$34 → ≈$8 예상. 선별 콜은 판정 규칙을 사용자 메시지에서 system으로 옮겨
