...
| people | **인물 프로필**(#112, 갱신은 admin만 #135): speaker_key(assembly_speeches.speaker 일치 키)·name(표시명)·kind(의원/정부·참고인)·party(활동 당시)·position·terms·is_22(현역 판정=22대 발언 존재)·speech_count·stance_summary(AI 쟁점별 입장 요약 캐시)·stance_updated_at. **등록 문턱(#164): 의원·위원장은 발언 1건, 통신사 임원은 무조건, 그 외 정부·참고인만 4건 이상.** RLS: select/update 공개(issues 관례) |
...
- **명부 등록 문턱(2026-09-13 완화, #164)**: 직함이 `MEMBER_POS`(위원·위원장·의원·간사·소위원장 등)이면 **발언 1건이라도 등록**(한 번만 출석한 위원도 소관 인물이다), **통신사 임원은 무조건 등록**, 그 외 정부·참고인만 종전대로 4건 이상. 224명(의원 125·정부참고인 99).
