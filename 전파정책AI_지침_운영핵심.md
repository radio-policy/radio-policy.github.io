...
- **법안 단계 라벨 규칙을 바꿔 배포할 때는 `assembly_crawler.py --suppress-status-alerts`로 1회 백필부터** — 파생 규칙만 바뀌어도 수백 건이 '상태변경'으로 판정돼 운영자 알림·구독자 큐가 폭주한다(실측 442건). 신규 법안 알림은 그대로 나간다. (배경역사 #122)
