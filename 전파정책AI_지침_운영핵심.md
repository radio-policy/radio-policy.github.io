...
- **대시보드 국회 법안 탭 = 두 줄 통계(2026-09-09, #139)**: 1줄 '할 일·시간축' 카드(전체·최근 발의 7일·의견등록 가능·**최근 변경 7일**), 2줄 **진행 단계 스트립** 발의 → 위원회 회부 → 위원회 심사 중 → 위원회 통과 → 법사위·본회의 → 본회의 통과 | 폐기·철회. 칸 키는 `app.js _billStageKey()`(접수/회부/심사/의결/법사위/통과/폐기)가 proc_result 라벨을 접은 것이고, 칸 이름은 카드 상태 라벨과 같은 말을 쓴다. 종전 '계류 중' 카드는 앞 다섯 칸의 합이라 없앴다. **'위원회 회부' 칸에 '의견등록 가능 N건' 배지**(국회 입법예고는 회부 직후~상정 전에 열리므로 이 칸이 의견 구간; N은 상단 카드와 같은 `_billCommentOpen` 판정 — 회부 정보가 API에 늦게 오는 며칠은 '발의' 칸 법안도 열려 있어 칸별로 나누지 않는다). 카드에는 7일 내 단계 변경을 '이전 → 현재 · M/D 변경'으로 표시(`_billChangedRecently`, prev_proc_result≠proc_result & updated_at). ⚠️ 9/4 백필 흔적(442건, prev=접수)은 '최근 변경'에 449건으로 잡혀 DB에서 prev를 현재값으로 맞춰 지웠다 — 억제 백필은 반드시 `--suppress-status-alerts`(prev까지 맞춤)로.
