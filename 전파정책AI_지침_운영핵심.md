...
| law_graph_edges | 법령 관계도 엣지(source_id→target_id, on delete cascade). relation_type: 근거(주제→법령)/인용(조문 인용)/하위법령(계열). source: seed/citation/family/**thdcmp**/ai. weight=인용·재확인 횟수(엣지 굵기). unique(source,target,relation_type). RLS 동일. **thdcmp(2026-08-02, #65)** = 법제처 3단비교 API(`lawService.do?target=thdCmp&knd=2`) 정본 위임 — description에 조문 근거("제69조 → 제17조") 기재, weight=4로 family(3)보다 우선. **CHECK 제약에 source 값을 추가해야 적재됨**(신규 source 태그 도입 시 `law_graph_edges_source_check` 확장 필수 — #65에서 누락으로 적재 실패 후 수정) |
...
- **`document_chunks`에 REINDEX 하지 말 것(디스크 증설 전까지)** — 255MB HNSW 재구축이 원본+사본+temp를 동시에 요구해 디스크가 터지고 **인스턴스가 통째로 읽기전용**이 된다(크롤러·브리핑 동반 중단). 기존 "대량 적재 후 REINDEX" 가드레일의 예외. kb_chunks는 가능. (배경역사 #69)
...
- **PostgREST 전량 조회에 페이지네이션 없이 `select()`만 쓰지 말 것** — 1,000행에서 잘린다. 총량이 상한을 넘는 첫날 조용히 터지며, 실제로 뉴스 1,086건 시점에 최신 86건이 누락돼 **이미 알림한 긴급 기사를 재발송**했다. `crawler._fetch_all_rows()` 패턴 사용. `.range()` 페이징에는 **반드시 `.order()`** — 정렬 없는 페이징은 행 누락이 비결정적이다. (배경역사 #66)
