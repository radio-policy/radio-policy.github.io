...
- **현행본 PDF 12건의 API 재적재는 하지 않는다.** 형식이 통일되고 law_id도 채워지지만, **PDF본에만 있는 별표를 잃는다**(적합성평가 고시 166청크 → 48청크). Phase 3는 기계 diff를 쓰지 않도록 설계돼 재적재가 필요 없다. 장래에 하더라도 별표가 실질인 **고시는 제외**하고 법률·시행령만, 기존 PDF본은 삭제하지 말고 `superseded`로 보존할 것.
