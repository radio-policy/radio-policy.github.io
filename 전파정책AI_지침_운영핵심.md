...
- **조문 단위 보기 (2026-09-06 신설 — 배경역사 #125, `lawmap_articles.js`)**. 주제 포커스 화면의 기본값. 동그라미=조문, 법령=조문을 담는 배경 상자(캔버스 `beforeDrawing`), 주제→근거 조문은 파란 점선, 회색 실선은 **근거 조문 본문이 다른 근거 조문을 실제로 인용하는 곳에만** 조문 라벨(`37조의2③ → 사업법 38조`)과 함께. 전체 인용망·법령 포커스는 종전(법령 단위) 그대로. 상태줄 토글 "조문 단위 보기 켬/끔"(localStorage `lawmap_article_mode`)으로 종전 주제 화면과 전환. `renderLawMapGraph` 첫 줄의 `lmaShouldHandle` 훅 한 곳에서 분기하고, 모델 구축이 실패하면 `_lmaBypass`로 종전 화면에 폴백(fail-open — 화면이 비는 것보다 낫다). 데이터는 노드 카드가 쓰는 `document_chunks` 조문 청크만, AI 호출 0회, 조회는 4개씩 병렬.
  - **근거 조문** = 주제 엣지 설명의 자기 조문(`lmaBasisKeys` — `lawmap_edge_check.own_articles`의 JS 이식, 나열·범위·교차인용 규칙 동일). 설명에 조문이 없고 문서가 있으면 제1조로 대표. **따라온 조문(1홉)** = 근거 조문이 인용한 화면 안 법령의 조문인데, **법률 본문이 인용한 것만** 동그라미로 올린다(준용·연계 = 실질 연결). 시행령·고시가 "법 제35조·제37조·제38조…"로 나열하는 것은 위임 근거 목록이라 다 그리면 상자가 넘친다(사업법 시행령 40조: 10개) → 조문 카드의 "이 조문이 인용하는 다른 조문"에만. 정의·목적 조항(「사업법」 제2조제18호 식 용어 참조)은 따라오지 않는다. 상자당 6개 상한(근거 우선).
