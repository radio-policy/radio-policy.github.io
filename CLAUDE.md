...
- *Citation verification* (#155, 2026-09-11): the `[원문 확인됨]` tag is **machine-verified, not model self-reported**. One shared file `supabase/functions/_shared/cite_verify.js` (plain JS, loaded by Deno, the browser via index.html, and `node tests/cite_verify.test.js`) does three things: `expandArticles` puts whole articles into the prompt when retrieval caught only a fragment (rag.ts before callSonnet / app.js before claudeFetch); `verifyCitations` parses each tag's citation (law name·조·항·호·별표) and checks the text was really in context, then a Haiku judge compares wording. Missing → `[⚠️ 원문 미확인 — 검색 결과에 해당 조문 없음]`, contradicted → `[⚠️ 원문과 다르게 설명됨 — 확인 필요]`, otherwise untouched (fail-open). Telegram runs it inside `answerAdvisory`; the dashboard calls the `verify-citations` Edge Function after the stream ends. Any new advisory entry point must go through both steps. Editing the shared file means redeploying `telegram-webhook` **and** `verify-citations` and bumping its cache-buster in index.html (GitLab CI copies it explicitly). Edge calls now log tokens to `api_usage` (host=edge; claude-proxy taps the stream, site label from the `x-site` header).
