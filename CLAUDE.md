...
- *AI advisory*: RAG 3-way hybrid — keyword `ilike`+`search_chunks_trgm`, plus Voyage embedding → `match_chunks_semantic` (pgvector) → hybrid rank → Claude Sonnet. `kb_documents` also holds pending-bill summaries (`concept_type='Bill'`, path `regulatory-kb/bills/<year>/<의안번호>.md`, #121) — surfaced with a `[법안요약 · 국회 계류 — 확정 법령 아님]` label, never as settled law.
