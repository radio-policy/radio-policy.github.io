...
- **push 대상은 `gitlab` 원격 하나다** (2026-08-20~, GitHub 계정 정지로 이관 — `gitlab.com/radio-policy/radio-policy.gitlab.io`). `origin`(GitHub)은 동결 — 정지 중 push 불가이며, **복구 후에도 주 저장소는 GitLab 유지, GitHub은 미러로 강등**(지침의 "GitHub 계정 복구 시 체크리스트" 참조). 대시보드도 GitLab Pages(`radio-policy.gitlab.io`)가 서빙한다. 인증은 remote URL이 아니라 **`C:\Users\SKTelecom\.gitlab-credentials` 파일**(gitlab.com 한정 credential store helper, 2026-08-21 — URL 평문 토큰 노출 방지). 토큰 교체 시 이 파일만 수정.
- **세션 커밋 규칙 3줄** (여러 창·맥북 공통): ① 작업 시작 전 `git pull gitlab main` ② 커밋·푸시는 작업한 세션이 직접 하되 **동시에 커밋하는 세션은 하나만** ③ 커밋 후 즉시 push + 원격 대조 — 미푸시를 쌓아두지 않는다.
