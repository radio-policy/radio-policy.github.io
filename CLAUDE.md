...
- **push 대상은 `gitlab` 원격 하나다** (2026-08-20~, GitHub 계정 정지로 이관 — `gitlab.com/radio-policy/radio-policy.gitlab.io`). `origin`(GitHub)은 동결 — 정지 중 push 불가이며, **복구 후에도 주 저장소는 GitLab 유지, GitHub은 미러로 강등**(지침의 "GitHub 계정 복구 시 체크리스트" 참조). 대시보드도 GitLab Pages(`radio-policy.gitlab.io`)가 서빙한다.
