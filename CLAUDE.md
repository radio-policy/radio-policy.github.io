...
**Scheduling is dual** — Supabase **pg_cron is the primary trigger** (GitHub cron drops jobs unreliably); GitHub Actions cron is backup. An external `health_watchdog.py` (independent of Supabase) distinguishes "broken" from "no news." See the pg_cron job table in the 지침. **PC scheduled tasks run on the home PC** (Windows, user account, 24h, `C:\Claude\Radio-policy\radio-policy-ai`; #179, 2026-09-20): gov chain 16:30 · `refetch_content` hourly :22 · briefing backup 09:40 · assembly summary 10:30 · CRMS sync 1st of month 16:30. The company laptop keeps the same five tasks plus the local preview **registered but disabled** (standby) — do not re-enable them while the home PC is healthy (two chains overlap `assembly_minutes` imports and double the Haiku spend); if the home PC is down, `schtasks /change /tn <name> /enable`. `health_watchdog.py` also watches the PC heartbeats `last_gov_notice_run` (26h) and `last_refetch_run` (3h) and reports them as "집 PC 확인".
...
# 회의록 오프라인(세션) 파이프라인 — Anthropic API 0회 (#120). 16:30~17:30 실행 금지(집 PC 16:30·회사 PC 17:00 gov 체인, #178), import는 단일·순차
...
- **Call Python as `py -3.12`** (launcher pinned to 3.12) in scheduler tasks, .bat files, and manual runs — a bare `python` **and `py -3`** resolve to Python 3.13 (installed 2026-06-30, no packages) and die with `ModuleNotFoundError`. All packages live in 3.12 only. (#22) Since #178 (2026-09-20, home-PC migration) no script hardcodes the install path or the repo folder: `.bat` = `cd /d "%~dp0"` + `py -3.12`, `run_hidden.py` = python.exe next to its own pythonw.exe + chdir to its folder, `setup_*.ps1` = `$PSScriptRoot` + pythonw resolved via `py -3.12`. Don't put `C:\Users\SKTelecom\…` back. The home PC (primary since #179) runs these tasks under the **user account**, not SYSTEM (SYSTEM + `pip --user` mismatch incident).
