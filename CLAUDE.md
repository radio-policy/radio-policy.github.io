...
- **GitHub PAT** lives in Supabase Vault `github_pat` (fine-grained, `radio-policy-commit-org`, resource owner = org `radio-policy`, expires 2027-09-04). Required scopes: Contents(R/W) + Metadata + **Actions(R/W)**. Fine-grained PATs are bound to the resource owner: moving the repo to another account/org invalidates the token immediately (403) and the two DB dispatch functions hardcode the repo path — reissue under the new owner and update both (#115). If Actions is missing, `git push` still works but `workflow_dispatch` returns 403 while pg_cron reports `succeeded` → all triggers stop **silently**. Verify a swap by `net._http_response.status_code` (204=ok), not by cron job status. (#18)
