...
(function (root) {
  ...
  function extractKeywords(text) {
    const words = String(text || '').split(/[\s,.·()[\]「」『』<>:;!?]+/)
      ...
      .map(function (w) { const s = w.replace(KW_JOSA, ''); return s.length >= 2 ? s : w; })
      .map(function (w) { const s = w.replace(VERB_TAIL, ''); return s.length >= 2 ? s : w; })
  ...
  function rankChunks(results, keywords, baseKeywords, query) {
    ...
    // 일반 가점·감점: 조문번호 있는 청크 = 법령·고시 원문 → 가점 / 파일 확장자 문서(.pdf/.md 등) = 논문·계획서류 → 감점
